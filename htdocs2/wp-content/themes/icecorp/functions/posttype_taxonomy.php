...
$args = array(
	'label'  => WORKS_POST_LABEL,
	//投稿タイプの名前
	'labels' => array(
		'singular_name'      => WORKS_POST_TYPE,//投稿タイプの名前
		'menu_name'          => WORKS_POST_LABEL,//メニュー（画面の左）に表示するラベル
		'all_items'          => WORKS_POST_LABEL . '一覧',
	...
	'rewrite'            => array( 'slug' => WORKS_PERMALINK_REWRITE ),
	//リライト設定
	/**
	 * 「true」の場合、（初期値： true ）
...
$args = array(
	...
	'rewrite'      => array( 'slug' => WORKS_TAX_CAT )
);
//利用する時はコメントアウト外して、第一引数にカスタム投稿タイプ名を設定すること。
register_taxonomy( WORKS_TAX_CAT, array( WORKS_POST_TYPE ), $args );


$args = array(
	'label'  => SERVICE_POST_LABEL,
	//投稿タイプの名前
	'labels' => array(
		'singular_name'      => SERVICE_POST_TYPE,//投稿タイプの名前
		'menu_name'          => SERVICE_POST_LABEL,//メニュー（画面の左）に表示するラベル
		'all_items'          => SERVICE_POST_LABEL . '一覧',
		'add_new_item'       => '新規投稿を追加',//新規作成ページの左上に表示されるタイトル
		'add_new'            => '新規追加',//メニュー（画面の左）の「新規」の位置に表示するラベル
		'new_item'           => '新規投稿',//一覧ページの右上にある新規作成ボタンのラベル
		'edit_item'          => '投稿を編集',//編集ページの左上にあるタイトル
	...
	//'menu_icon' => true,//カスタム投稿のメニューに表示するアイコンのURLを指定
	//'capability_type' => 'post', // 権限を区別するための投稿タイプ名を指定
	'rewrite'            => array( 'slug' => SERVICE_PERMALINK_REWRITE ),
	//リライト設定
	/**
	 * 「true」の場合、（初期値： true ）
	 * 「http://ブログのアドレス / カスタム投稿タイプ名 / 個々のカスタム投稿のスラッグ / 」でアクセスできる。
...
$args = array(
	'label'  => NEWS_POST_LABEL,
	//投稿タイプの名前
	'labels' => array(
		'singular_name'      => NEWS_POST_TYPE,//投稿タイプの名前
		'menu_name'          => NEWS_POST_LABEL,//メニュー（画面の左）に表示するラベル
		'all_items'          => NEWS_POST_LABEL . '一覧',
		'add_new_item'       => '新規投稿を追加',//新規作成ページの左上に表示されるタイトル
		'add_new'            => '新規追加',//メニュー（画面の左）の「新規」の位置に表示するラベル
		'new_item'           => '新規投稿',//一覧ページの右上にある新規作成ボタンのラベル
		'edit_item'          => '投稿を編集',//編集ページの左上にあるタイトル
